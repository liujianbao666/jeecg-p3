...
function getUserinfo(){
	...
	$.ajax({
		...
		type: 'get',
		dataType : "json",
		url: "org.do?getUserInfo",//请求的action路径
		error: function () {//请求失败处理函数
			alert('请求失败');
		},
		success:function(data){ //请求成功后处理函数。  
			user = data;   //把后台封装好的简单Json格式赋给treeNodes
		}
	});
	return user;
}
function getUserOrgTree(orgNo){
	var treeDataList;
    $.ajax({
   		async : false,
   		cache:true,
   		type: 'POST',
   		dataType : "json",
   		url: "org.do?myQuery&pid="+orgNo,//请求的action路径
   		error: function () {//请求失败处理函数
   			alert('请求失败');
   		},
   		success:function(data){ //请求成功后处理函数。  
 ...
 * @returns
 */	
 function orgSearch(zTreeId, searchField, isHighLight, isExpand){
	...
	function ztreeFilter(zTreeObj,_keywords,callBackFunc) {
		...
		if(nodesShow.length==0){    //匹配结果为0 更新缓存中的组织机构树
			if(cacheFlag==0){
				cacheFlag = 1;
				updateOrgCache();
			}
 ...
 function updateOrgCache(){
	 ...
	 if(user){
	    ...
	    $.ajax({
	   		...
	   		cache:false,
	   		type: 'get',
	   		url: "org.do?updateOrgCache&pid="+pid,//请求的action路径
	   		error: function () {//请求失败处理函数
	   			alert('请求失败');
	   			cacheFlag = 0;
	   		},
	   		...
	   		console.log("组织机构缓存更新状态："+data.status);
	   		cacheFlag = 0;
	   		}
	   	});
