...
function refreshRunTable(){
   ...
   var endTime=  $("#end_time").val();
    ...
    var opt = {
        url : "ChargingPileRunController.do?getChargingPlanList&orgNo="+orgNo+"&flowNo="+flowNo
        +"&startTime="+startTime+"&endTime="+endTime+"&tgNo="+tgNo,
    };
    var opt = "";
...
function TableInit() {
    $("#planTable").bootstrapTable({
        url: 'ChargingPileRunController.do?getChargingPlanList',         //请求后台的URL（*）
        method: 'get',                      //请求方式（*）
        ...
        columns: [  
            ...
            width:50,
                ...
                formatter:function(value, row, index){
            		if(row.flowNo){
                		var url="planDetailsController.do?planDetails&flowNo="+row.flowNo;
                		var title = "充电计划详情";
