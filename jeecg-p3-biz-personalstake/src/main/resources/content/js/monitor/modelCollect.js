...
$(document).ready(function(){
	...
	//初始化列表
	//tgEcInit();  //初始化控制器列表
	ecParamInit(); $('#ecParamInfo').colResizable(); //拖动改变列宽
	tgParamInit();
	routParamInit();
	//tgForecastInit();
	tgForeChartInit();
	...
	$("#search-btn").bind("click",function(){
		...
		case "1":
			...
			//召测数据，刷新对应的表格
			refreshTable("ecParamInfo",tgNos,type);
			break;
		case "2":
			$(".tgParamDiv").css('display','block');
			refreshTable("tgParamInfo",tgNos,type);
			break;
		case "3":
			...
			//$(".tgRouterDiv").css('display','none');
			$(".routParamDiv").css('display','block');
			refreshTable("routParamInfo",tgNos,type,serialNo);
			break;
		case "4":
			$("#search-btn").removeAttr("disabled");
			...
			refreshFCharts();
			//refreshTable("tgForecast",orgNos,tgNos,ecNos,type);
		}
			break;
		case "5":
			$("#search-btn").removeAttr("disabled");
			...
			break;
		/*case "5":
			$(".ecClickDiv").css('display','block');
			refreshTable("ecClickInfo",tgNos,type);
			break;*/
		case "6":
			$(".routClickDiv").css('display','block');
			refreshTable("routClickInfo",tgNos,type);
			break;
		default:
			break;
		}
...
//判断为空
	$('#paramType').change(function(){
		...
		else if(tgNo == "" || tgNo == undefined || tgNo == null){
			quickNotify("请先选择台区！","warning");
			//return false;
		}
		else{
			//路由器档案参数
...
function refreshTable(id,tgNos,type,serialNo){
	if(type == "3"){
		var opt = {
				url : "monitorController.do?getCollectData&tgNos="+tgNos+"&type="+type+"&serialNo="+serialNo,
			};
			$('#'+id).bootstrapTable('refresh', opt); 
	}
	else{
		/*var opt = {
				url : "monitorController.do?getCollectData&tgNos="+tgNos+"&type="+type,
			};
			$('#'+id).bootstrapTable('refresh', opt); */
		$.ajax({   
        ...
        success : function(datas) {//返回list数据并循环获取  
           ...
           // var select = $("#tgNameSelect");  
            for (var i = 0; i < datas.length; i++) {  
            	$("#tgNameSelect").append("<option value='"+datas[i].tgId+"'>"  
