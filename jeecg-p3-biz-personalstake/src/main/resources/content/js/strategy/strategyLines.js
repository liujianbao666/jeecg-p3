...
$(document).ready(function(){
	...
	$("#charts").height(($("body").height() * 0.8) + "px")
	selectInit();
	init();
	timeInit();
	var user = getUserOrg();
...
function refreshTgCurves(){
	...
	$.ajax({   
		  ...
		  url: "strategyConfigController.do?getStrategyLines",//后台处理程序,获取显示数据    	
		  error: function () {//请求失败处理函数   
			  return false;
		  },   
...
//		      option.series[1].data = data.elePrices;  
		      option.series[1].data = data.piesPower;
		      option.series[2].data = data.usersPower;
