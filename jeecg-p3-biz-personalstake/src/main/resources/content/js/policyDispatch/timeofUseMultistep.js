...
$(document).ready(function(){
	...
	$("#add-btn").bind("click",function(){
		...
		var dialog = new BootstrapDialog({
	        ...
	        closable: false,
	        data: {
	            'pageToLoad': 'multistepElectrictyPriceController.do?addMultistep'
	        },
	        buttons: [{
	            label: '保存',
	...
	$("#edit-btn").bind("click",function(){
		...
		if(selects.length == 0 || selects.length > 1){
			...
			var dialog = new BootstrapDialog({
		        ...
		        closable: false,
		        data: {
		            'pageToLoad': 'multistepElectrictyPriceController.do?modify'
		        },
		        buttons: [{
		            label: '保存',
		            cssClass: 'btn-primary',
	...
	$("#remove-btn").bind("click",function(){
		...
		if(selects.length == 0 || selects.length > 1){
			...
			var dialog = new BootstrapDialog({
		        ...
		        closable: false,
		        data: {
		            'pageToLoad': 'policyModelController.do?deleteStepPrice'
			        },
			        buttons: [{
			            label: '确认删除？',
			            cssClass: 'btn-primary',
	...
	$('#orgName').bind("click",function(){
		...
		var dialog = new BootstrapDialog({
	        ...
	        closable: false,
	        data: {
	          //  'pageToLoad': 'multistepElectrictyPriceController.do?orgNoSelect'
	        	'pageToLoad': 'policyModelController.do?orgNoSelect&userOrg='+userOrg
	        },
	        buttons: [{
	            label: ' 确认',
	            cssClass: 'btn-primary',
...
function refreshTable(){
	var orgNo = $('#orgNo').val();
	var opt = {
		url : "policyModelController.do?getStepElePriceTableData&orgNo="+orgNo,
	};
...
/*function refreshChart(){
	...
	$.ajax({   
		  ...
		  url: "multistepElectrictyPriceController.do?getPriceChartItem",//后台处理程序,获取显示数据    	
...
function initTableInfo(tg_id_init){
	$('#priceDg').bootstrapTable({    
	    url:'policyModelController.do?getStepElePriceTableData',
	   // striped: true,                      //是否显示行间隔色
	    ...
	    columns:[  
	        ...
	        {field:'step',halign:"center",title:'阶梯等级',align:"right"},
	        {field:'powerbegin',halign:"center",title:'阶梯电量起始值',align:'center'},
	        {field:'powerend',halign:"center",title:'阶梯电量结束值',align:'center'},
	        {field:'price',halign:"center",title:'阶梯电价',align:'center'},
	        {field:'year',halign:"center",title:'年',align:'center'},
	        {field:'monthbegin',halign:"center",title:'起始月份',align:'center'},
	        {field:'monthend',halign:"center",title:'结束月份',align:'center'}
	    ],
