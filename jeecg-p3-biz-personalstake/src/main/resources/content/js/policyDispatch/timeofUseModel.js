...
$(document).ready(function(){
	...
	$("#add-btn").bind("click",function(){
		...
		var dialog = new BootstrapDialog({
	        ...
	        closable: false,
	        data: {
	            'pageToLoad': 'policyModelController.do?add'
	        },
	        buttons: [{
	            label: '保存',
	...
	$("#edit-btn").bind("click",function(){
		...
		if(selects.length == 0 || selects.length > 1){
			...
			var dialog = new BootstrapDialog({
		        ...
		        closable: false,
		        data: {
		            'pageToLoad': 'policyModelController.do?modify'
		        },
		        buttons: [{
		            label: '保存',
		            cssClass: 'btn-primary',
	...
	$("#remove-btn").bind("click",function(){
		...
		if(selects.length == 0 || selects.length > 1){
			...
			var dialog = new BootstrapDialog({
		        ...
		        closable: false,
		        data: {
		            'pageToLoad': 'policyModelController.do?deletePrice'
			        },
			        buttons: [{
			            label: '确认删除？',
			            cssClass: 'btn-primary',
	...
	$('#orgName').bind("click",function(){
		...
		var dialog = new BootstrapDialog({
	        ...
	        closable: false,
	        data: {
	          //  'pageToLoad': 'policyModelController.do?orgNoSelect'
...
function refreshTable(){
	var orgNo = $('#orgNo').val();
	var opt = {
		url : "policyModelController.do?getTableData&orgNo="+orgNo,
	};
...
/*function refreshChart(){
	...
	$.ajax({   
		  ...
		  url: "policyModelController.do?getPriceChartItem",//后台处理程序,获取显示数据    	
...
function initTableInfo(tg_id_init){
	$('#priceDg').bootstrapTable({    
	    url:'policyModelController.do?getTableData',
...
//	        {field:'end',halign:"center",title:'时段(停止)',align:'center'},
	        ...
	        {field:'enddate',halign:"center",title:'截止日期',align:'center'}
	    ],
