...
function refreshRunTable() {
	...
	var opt = {
		url : "plan.do?getHistoryPlanList&orgNo=" + orgNo + "&flowNo=" + flowNo
				+ "&startTime=" + startTime + "&endTime=" + endTime + "&tgNo="
				+ tgNo,
	};
	var opt = "";
...
function TableInit() {
	$("#planTable")
			.bootstrapTable(
					{
						url : 'plan.do?getHistoryPlanList', //请求后台的URL（*）
						method : 'get', //请求方式（*）
						...
						/*onLoadSuccess: function (data) {
						    var data = $('#planTable').bootstrapTable('getData', true);
						    // 合并单元格
						    var fieldList=["odId","tgName","stakeNo"];
						    mergeCells(data, "odId", 1, $('#planTable'),fieldList);
						},*/
						...
						columns : [
								...
								},
								{
									...
									formatter : function(value, row, index) {
										if (row.flowno) {
											var url = "personalstake/plan.do?planDetail&orderId="
													+ row.flowno;
											var title = "充电计划详情";
											return '<a href="#" onclick="clickT(\''
													+ url
													+ '\',\''
													+ title
													+ '\')" style="color:blue">'
