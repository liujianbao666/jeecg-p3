...
public class P3OneToMainUtil {
	...
	public static void main(String[] args) {
		...
		codeParamEntityIn.setProjectName("personalstake");	//工程名
		codeParamEntityIn.setEntityPackage("chargepile");	 //包名
		
		codeParamEntityIn.setTableName("ev_cg");//主表名
		codeParamEntityIn.setEntityName("EvCg");	 //实体名
		codeParamEntityIn.setFtlDescription("充电桩档案");	 //描述
		
		//第二步：设置子表集合配置
		List<SubTableEntity> subTabParamIn = new ArrayList<SubTableEntity>();
		//[1].子表一
		SubTableEntity po = new SubTableEntity();
		po.setTableName("ev_cg_run");//子表名
		po.setEntityName("EvCgRun");	    //实体名
		po.setEntityPackage("chargepile");	        //包名
		po.setFtlDescription("充电桩运行信息表");       //描述
		//子表外键参数配置
		po.setForeignKey("stakeno");//子表外键
		po.setMainForeignKey("stakeno");//对应的主表字段
